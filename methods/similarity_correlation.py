...
class SimilarityCorrelation:
    """This class implements the similarity correlation method."""
    ...
    def __init__(self, ABX, baseline, post_ABX_container, baseline_ref, method, timepoints, iters=None, new=True,
        ...
        ABX: pandas dataframe of shape (# test subjects, # species):
                 Represent the antibiotic treatment samples of the tested subjects and the row names are the
                 identifiers of the tested subjects.

        baseline: pandas dataframe of shape (# test subjects, # species):
                      represent the baseline samples of the tested subjects and the row names are the identifiers of
                      the tested subjects.

        post_ABX_container: dictionary:
                                dictionary of the post antibiotic treatment samples of the tested subjects.
                                The keys are the identifiers of the tested subjects and the values are numpy matrices
                                 of shape (# time points, # species).

        keys_ref: list of strings:
                      The identifiers of the reference subjects (can overlap with the tested subjects keys).

        baseline_ref: pandas dataframe of shape (# ref subjects, # species):
                          represent the baseline samples of the reference subjects and the row names are the identifiers
                          of the reference subjects.

        method: string:
                    the method to calculate the similarity correlation. Choose from ' Jaccard' and 'Dice'.

        timepoints: Integer:
                        the number of time points after antibiotics administration the returned species considered as
                         survived.

        iters: Integer:
                   The number of synthetic samples to generate. If None, no synthetic samples are generated, and the
                    zscore parameter should be False.

        new: Boolean:
                 If True, the correlation is calculated w.r.t the new species, if False, the correlation is calculated
                 w.r.t the returned species (at time points >timepoints).

        strict: Boolean:
                    if True, the returned species at time t are the species that are present in the baseline,
                    absent in the antibiotic treatment and present in all the post antibiotic samples where
                    t >= timepoints. If False, the returned species are the same except that they are present in time
                    t = timepoints and can be absent in the post antibiotic samples at time t > timepoints except of the
                    last sample in test_post_ABX_matrix.

        n_jobs: Integer:
                    The number of parallel jobs to run.

        zscore: Boolean:
                    If True, the z-score method is applied.
        """
        (self.ABX, self.baseline, self.post_ABX_matrices, self.baseline_ref,
         self.method, self.timepoints, self.iters, self.new, self.strict,
         self.keys, self.keys_ref, self.n_jobs, self.zscore, self.verbose) = self._validate_input(ABX, baseline,
                                                                                                  post_ABX_container,
                                                                                                  baseline_ref, method,
                                                                                                  timepoints, iters,
                                                                                                  new, strict, n_jobs,
    ...
    def _validate_input(self, ABX, baseline, post_ABX_container, baseline_ref, method, timepoints, iters, new, strict,
        ...
        if not isinstance(zscore, bool):
            raise ValueError("zscore must be of type bool")
        ...
        return (ABX.values.T, baseline.values.T, post_ABX_matrices, baseline_ref.values.T, method, timepoints, iters,
                new, strict, keys, keys_ref, n_jobs, zscore, verbose)
    ...
    def calc_similarity(self):
        ...
        for base, abx, post_mat, key in zip(self.baseline, self.ABX, self.post_ABX_matrices, self.keys):
            if self.verbose:
                print(f"Processing subject: {key}...")
            ...
            for j, (base_ref, key_ref) in enumerate(zip(self.baseline_ref, self.keys_ref)):
                if key_ref != key:
                    ...
                    if self.zscore:
                        ...
                        sims[key_ref] = (sim_new, sim_others)
    ...
    def _generate_synthetic_cohorts(self):
        ...
        shuffled_baseline_lst = Parallel(n_jobs=self.n_jobs)(
            delayed(_create_synthetic_cohort)(self.baseline_ref.astype(np.float64)) for _ in range(self.iters))
