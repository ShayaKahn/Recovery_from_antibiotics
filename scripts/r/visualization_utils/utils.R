...
LR_plot_outline <- function(data, x_title, y_title,
                            size = 8, size_line = 5,
                            linewidth_axis = 3,
                            size_title = 60,
                            size_text = 35,
  ...
  p <- ggplot(data, aes(x = x, y = y)) +
    ...
    theme(
      ...
      axis.line = element_line(color = "black", linewidth = linewidth_axis),
      axis.title = element_text(size = size_title, color = "black"),
      axis.text = element_text(size = size_text, color = "black")
    )
  
  if (show_lm) {
    p <- p + geom_smooth(method = "lm", se = TRUE, color = line_color, size = size_line)
  }
