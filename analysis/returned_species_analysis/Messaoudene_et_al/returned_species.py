...
from src.host_specific_recovery.io.Messaoudene_et_al_loader import (
...
def run_Messaoudene_returned_species_analysis(
        subject_id,
        non_rarefied=True,
...
def run_Messaoudene_non_rarefied_returned_species_analysis(
        subject_id,
        survived_reduction_threshold=10.0,
):
    """Run the returned-species analysis with non-rarefied abundances."""
    return run_Messaoudene_returned_species_analysis(
        subject_id,
        non_rarefied=True,
        survived_reduction_threshold=survived_reduction_threshold,
    )
...
def plot_Messaoudene_returned_species(outputs, log_scale=True, top_x=7, path=None,
                                      show_legend=False, show_low_count_annotation=False):
    """Plot returned-taxon trajectories for one Messaoudene subject."""
    return plot_returned_species_analysis(
        ...
        legend_bbox_to_anchor=(0.39, 0.54),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "subject_id",
        nargs="?",
        help="Subject ID. Omit it to analyze all subjects.",
    )
    parser.add_argument(
        "--linear",
        action="store_true",
    ...
    for subject_id in subject_ids:
        ...
        figure, _ = plot_Messaoudene_returned_species(
            outputs,
