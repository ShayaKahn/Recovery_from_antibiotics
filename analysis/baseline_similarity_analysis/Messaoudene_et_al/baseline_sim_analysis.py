...
results_sda = run_surrogate_analysis(dataset, timepoints_val=2, method="jensenshannon")
plot_results = plot_SDA(outputs=results_sda, fig_size=(9, 4), sur_color="#8C2928", real_color="#51A246",
                        ...
                        y_title='Standardized Jaccard similarity', naive=True)

order = np.array(plot_results["order"])

labels = np.array(dataset["filtered_keys"])[order]

df_baseline_full = dataset["baseline_full_df"]
df_baseline = dataset["baseline_df"]

baseline_full_rjsd = plot_pcoa_and_distance_heatmap(
    df_baseline_full,
    metric="rjsd",
    title_prefix="Full baseline"
)
