...
metadata <- read_csv("C:/Users/USER/OneDrive/Desktop/Antibiotics/DAV132/Metadata.csv")

run_name <- metadata[,c("Run", "Name", "Assay Type")]

run_name_amplicon <- run_name[run_name[["Assay Type"]] == "AMPLICON", , drop = FALSE]

rename_map <- setNames(run_name_amplicon$Name, run_name_amplicon$Run)

biom <- read_biom("C:/Users/USER/OneDrive/Desktop/Antibiotics/DAV132_picrust2_12_12_25_subset/feature-table.biom")

mat <- as.matrix(biom_data(biom))
biom_df <- as.data.frame(mat)

old <- names(biom_df)
names(biom_df) <- ifelse(old %in% names(rename_map), unname(rename_map[old]), old)

data_biom <- biom_df

data_biom <- data_biom %>% mutate(across(everything(), ~ as.numeric(as.character(.x))))

write.csv(data_biom, 
          file = 'C:/Users/USER/OneDrive/Desktop/Antibiotics/DAV132/picrust2_data/full_ASV_table.csv',
          row.names = TRUE)
...
# CZA subjects
data_subject_10_CZA <- create_subject_df(data_biom, 10)
data_subject_18_CZA <- create_subject_df(data_biom, 18)
data_subject_32_CZA <- create_subject_df(data_biom, 32)
data_subject_40_CZA <- create_subject_df(data_biom, 40)
data_subject_57_CZA <- create_subject_df(data_biom, 57)
data_subject_65_CZA <- create_subject_df(data_biom, 65)
data_subject_77_CZA <- create_subject_df(data_biom, 77)
data_subject_85_CZA <- create_subject_df(data_biom, 85)
data_subject_108_CZA <- create_subject_df(data_biom, 108)
data_subject_123_CZA <- create_subject_df(data_biom, 123)
data_subject_142_CZA <- create_subject_df(data_biom, 142)
data_subject_5117_CZA <- create_subject_df(data_biom, 5117)

# PTZ subjects
data_subject_9_PTZ <- create_subject_df(data_biom, 9)
data_subject_15_PTZ <- create_subject_df(data_biom, 15)
data_subject_34_PTZ <- create_subject_df(data_biom, 34)
data_subject_41_PTZ <- create_subject_df(data_biom, 41)
data_subject_51_PTZ <- create_subject_df(data_biom, 51)
data_subject_72_PTZ <- create_subject_df(data_biom, 72)
data_subject_75_PTZ <- create_subject_df(data_biom, 75)
data_subject_93_PTZ <- create_subject_df(data_biom, 93)
data_subject_105_PTZ <- create_subject_df(data_biom, 105)
data_subject_112_PTZ <- create_subject_df(data_biom, 112)
data_subject_122_PTZ <- create_subject_df(data_biom, 122)

# CRO subjects
data_subject_3_CRO <- create_subject_df(data_biom, 3)
data_subject_23_CRO <- create_subject_df(data_biom, 23)
data_subject_25_CRO <- create_subject_df(data_biom, 25)
data_subject_38_CRO <- create_subject_df(data_biom, 38)
data_subject_52_CRO <- create_subject_df(data_biom, 52)
data_subject_67_CRO <- create_subject_df(data_biom, 67)
data_subject_78_CRO <- create_subject_df(data_biom, 78)
data_subject_89_CRO <- create_subject_df(data_biom, 89)
data_subject_107_CRO <- create_subject_df(data_biom, 107)
data_subject_118_CRO <- create_subject_df(data_biom, 118)
data_subject_125_CRO <- create_subject_df(data_biom, 125)
data_subject_140_CRO <- create_subject_df(data_biom, 140)

data_baseline_full <- create_time_point_df(data_biom, 1)
data_day_3_full <- create_time_point_df(data_biom, 3)
data_ABX_full <- create_time_point_df(data_biom, 6)
data_day_9_full <- create_time_point_df(data_biom, 9)
data_day_12_full <- create_time_point_df(data_biom, 12)
data_day_16_full <- create_time_point_df(data_biom, 16)
data_day_25_full <- create_time_point_df(data_biom, 25)
data_post_ABX_full <- create_time_point_df(data_biom, 37)
...
dir_path <- 'C:/Users/USER/OneDrive/Desktop/Antibiotics/DAV132/picrust2_data'
