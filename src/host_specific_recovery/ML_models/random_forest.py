...
class MicrobiomeRandomForestRegressor:
    """Random forest regressor for microbiome abundance tables.

    Mirrors MicrobiomeRegularizedLinearRegressor's preprocessing, combined-table
    handling, hyperparameter search, and SDA target-leakage avoidance, swapping
    only the final estimator for a scikit-learn RandomForestRegressor. Microbial
    branches receive prevalence filtering, an optional abundance transformation,
    and standardization. Continuous metadata columns receive median imputation
    and standardization. Numeric binary metadata columns receive most-frequent
    imputation without scaling, while categorical columns receive most-frequent
    ...
    def __init__(self, min_prevalence=0.1, transform="none", pseudocount=1e-6,
                 ...
                 n_estimators=1000, max_depth=None, min_samples_leaf=2, max_features="sqrt",
    ...
    def _validate_parameters(self):
        ...
        if not isinstance(self.min_samples_leaf, int) or self.min_samples_leaf <= 0:
            raise ValueError("min_samples_leaf must be a positive integer.")
        if not (
            self.max_features in {"sqrt", "log2", None}
            or isinstance(self.max_features, int)
            or isinstance(self.max_features, float)
        ):
            raise TypeError("max_features must be 'sqrt', 'log2', None, int, or float.")
        if isinstance(self.max_features, int) and self.max_features <= 0:
            raise ValueError("integer max_features must be positive.")
        if isinstance(self.max_features, float) and not (0 < self.max_features <= 1):
    ...
    def _build_pipeline(self, X):
        ...
        return Pipeline(steps=[("preprocessor", preprocessor), ("random_forest", self._build_random_forest_model())])
    ...
    def _validate_X(X):
        ...
        taxonomic_columns, functional_columns, _ = (
            MicrobiomeRandomForestRegressor._split_column_types(X)
        )
    ...
    def feature_importance(self, top_n=None):
        ...
        random_forest = self.pipeline_.named_steps["random_forest"]
